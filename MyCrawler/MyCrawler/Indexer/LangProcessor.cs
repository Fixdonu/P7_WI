...
namespace MyCrawler
{
    class LangProcessor
    {
        ...
        List<string> RemoveStopWord(string[] tokenList)
        {
        ...
        public void BinIndex(String dirPath)
        {
            ...
            foreach (var fp in Directory.GetFiles(dirPath))
            {
                foreach (var item in RemoveStopWord(TokenFromFile(fp)))
                {
                    ...
                    if (binInverse.TryGetValue(item, out docList))
                    {
                        docList[0]++;
                        if (docList.Last() != fNo)
                        {
